  ...
  return (
    <main className="max-w-5xl mx-auto py-12 px-4">
      <section className="mb-16">
        ...
        <p className="text-gray-600">최신 소식과 생각을 공유합니다.</p>
      </section>
      ...
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <aside className="w-full md:w-48 shrink-0">
          ...
          <nav className="flex flex-wrap md:flex-col gap-1">
            <Link
              href="/blog"
              className="px-4 py-2 rounded-lg bg-gray-100 text-black text-sm font-semibold transition-all"
            >
              All Posts
            </Link>
            {categories.map((category) => (
              <Link
                ...
                className="px-4 py-2 rounded-lg text-gray-500 hover:bg-gray-50 hover:text-gray-900 text-sm transition-all"
              >
                {category.name}
              </Link>
            ))}
        ...
        <section className="flex-1">
          ...
          <div className="divide-y divide-gray-100">
            {recentPosts.length > 0 ? (
              ...
              <p className="text-gray-500 py-12">아직 작성된 글이 없습니다.</p>
            )}
          </div>
        </section>
      </div>
